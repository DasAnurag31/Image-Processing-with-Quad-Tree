...
async function main() {
    ...
    const image = await loadImage('input.jpg'); // Replace 'input_image.jpg' with the path to your image file
    if (!image) {
    ...
    const maxDepth = 7; // Adjust the maximum depth as needed
    const errorThreshold = 30; // Adjust the error threshold as needed
    const quadtree = new Quadtree(imageData, maxDepth, errorThreshold);

    // Convert the image data using the quadtree
    convertImage(quadtree.root, ctx);

    // Save the compressed image to a file
    const outputFileName = 'outp.jpg'; // Output file name
...
function convertImage(node, ctx) {
    if (node.color !== null) {
        // Fill the region defined by the node with its average color
